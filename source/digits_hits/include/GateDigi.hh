...
public:
      ...
      //@{
      inline void  SetSourceType(G4int sourceType) { m_sourceType = sourceType; }
      inline G4int GetSourceType() const { return m_sourceType; }

      inline void  SetDecayType(G4int decayType) { m_decayType = decayType; }
      inline G4int GetDecayType() const { return m_decayType; }

      inline void  SetGammaType(G4int gammaType) { m_gammaType = gammaType; }
      inline G4int GetGammaType() const { return m_gammaType; }

      inline void  SetDecayIndex(G4int decayIndex) { m_decayIndex = decayIndex; }
      inline G4int GetDecayIndex() const { return m_decayIndex; }
      //@}

      //! Number of Compton and Rayleigh interactions along the photon path; filled by
      //! GateMultiPhotonAnalysis only, so it stays at -1 on the classical GateAnalysis path
      inline void  SetNInteractions(G4int nInteractions) { m_nInteractions = nInteractions; }
...
public:
  ...
  G4int m_decayIndex;             //!< decay channel index; -1 means 'not known'
  G4int m_nInteractions;          //!< # of Compton and Rayleigh interactions; -1 when not computed
